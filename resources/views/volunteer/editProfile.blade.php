...
<h2 class="text-3xl font-bold text-center mb-8 text-gray-800">Edit Profile, Manage Qualifications & Interests</h2>
    ...
    <div id="editProfile" class="bg-white p-6 rounded-lg shadow visible-section">
        ...
        <form method="POST" action="{{ route('volunteer.update-profile') }}" enctype="multipart/form-data" onsubmit="handleUpdate(event)">
            ...
            <div class="flex justify-center">
                <button type="submit" class="bg-blue-600 text-white px-5 py-2.5 rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 transition-all">
                    Update Profile
                </button>
            </div>
        </form>
    ...
    <div id="manageQualifications" class="bg-white p-6 rounded-lg shadow hidden-section">
        ...
        <div class="mb-4">
            ...
            @if($volunteer->vQualification)
                @php
                    $qualifications = json_decode($volunteer->vQualification);
                @endphp
        ...
        <form action="{{ route('volunteer.add-qualification') }}" method="POST" enctype="multipart/form-data" onsubmit="handleAdd(event)">
            @csrf
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">Upload New Qualifications (optional)</label>
                <input type="file" name="vQualification[]" id="vQualification" multiple class="mt-1 block w-full" onchange="toggleAddButton()">
            </div>

            <!-- Submit Button -->
            <div class="flex justify-center">
                <button type="submit" id="addQualificationButton" class="bg-gray-400 text-white px-5 py-2.5 rounded-lg cursor-not-allowed" disabled>
                    Add Qualifications
                </button>
            </div>
        </form>
    ...
    <div id="updateInterests" class="bg-white p-6 rounded-lg shadow hidden-section">
        ...
        <form method="POST" action="{{ route('volunteer.update-interests') }}" onsubmit="handleUpdate(event)">
            ...
            <div class="flex justify-center mt-8">
                <button type="submit" class="bg-yellow-500 text-white px-5 py-2.5 rounded-lg hover:bg-yellow-600 focus:ring-4 focus:ring-yellow-300 transition-all">
                    Update Interest
                </button>
            </div>
        </form>
